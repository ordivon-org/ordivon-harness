...
class HarnessPriorAttemptEvidence:
    """Exact pre-run authority paired with the terminal receipt it produced."""
    ...
    def from_dict(cls, value: dict[str, Any]) -> HarnessPriorAttemptEvidence:
        if set(value) != {"compiledAttempt", "receipt"}:
...
def admit_harness_agent_strategy(
    ...
    receipt_refs = {item.ref: item for item in context.consumption.receipt_refs}
    for adopted in strategy.adopted_context_refs:
        available = receipt_refs.get(adopted.ref)
        if available is None or available != adopted:
            raise ValueError(
                "Harness Agent Strategy adopted Context is not an exact prior receipt reference"
            )
    return matches[0]
