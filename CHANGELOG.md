...
- Tool-bearing WorkingView Runs can now recover transient cognition across a fresh process at the existing durable Tool-effect Snapshot boundary: Continuity derives the current WorkingSet-attempt boundary from Journal order, reconstructs complete Provider-authored Tool exchanges from exact completed Provider results plus terminal Tool intent/receipt/Observation evidence, reconciles already-completed effects without redispatch, excludes predecessor-attempt exchanges after a WorkingSet transition, and fails closed when the Contract did not retain exact Tool content; no new SQLite schema or generic conversation store was added;
