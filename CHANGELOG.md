...
- the mature Agent Loop now has an internal per-turn Working View projection seam: a committed WorkingSet can project a different exact model-visible view for each Provider turn without replacing canonical Run history, and Provider claim fences stale projected views before physical dispatch; Context discovery/relevance policy and Agent-owned WorkingSet transitions remain intentionally outside this slice and the projector is not in the recommended public facade;
