...
- the mature Agent Loop now separates canonical Run history from an exact per-turn Working View and can optionally accept an Agent-authored `AgentWorkingSetTransitionProposal`: the Agent chooses already-known exact successor pins, Continuity atomically replans/selects/commits them against the source Working View, competing transitions are fenced, and exact replay is idempotent; cognition transition is distinct from Runtime Tools and may run with `maxToolCalls=0`, while Context discovery/relevance/RAG/Memory remain outside the current slice and the execution seams are not in the recommended public facade;
