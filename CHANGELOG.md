...
- the mature Agent Loop now separates canonical Run history, durable Agent-selected WorkingSet and transient Tool-observation overlays: external/domain discovery may expose candidates without silently selecting them, Provider admission requires the committed WorkingView as an exact prefix plus only bound Tool Observation projections, raw discovered content must be privacy-aware materialized before it can become an exact pin, and Agent-authored successor transitions bind both the source WorkingSet and the exact effective model view they came from; Harness still owns no Context discovery/ranking/RAG/Memory policy and these cognition execution seams remain outside the recommended public facade;
