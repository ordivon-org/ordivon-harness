...
- replaced a mechanically fragile nested discovery-result function schema with a thin structured `decisionJson` envelope plus strict caller-side JSON/action admission; this is research action encoding rather than a supported API. P5 changed no Harness product source and did not resolve the separate no-Tool conclusion-correction failure; the accepted 12 decision calls still consumed 239,953 total tokens, leaving model-call/token efficiency as the next frontier;
