...
HARNESS_RUN_ID = "harness-run:repository-repair-edit-runtime-acceptance"
...
def _args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the frozen independent repository-repair edit Runtime bridge acceptance"
    )
    parser.add_argument(
        "--runtime-endpoint",
...
def _contract(created_at_ms: int) -> HarnessRunContract:
    return HarnessRunContract(
        ...
        harness_implementation_id="ordivon-harness@repository-repair-edit-runtime-acceptance",
        caller_id="caller:formal-runner-acceptance",
        caller_run_ref="trial:repository-repair-edit-runtime-acceptance",
        objective_ref=HarnessBoundReference(
            "objective:repository-repair-edit-runtime-acceptance", "objective", DIGEST_A
        ),
        context_refs=(
            HarnessBoundReference(
                "context:repository-repair-edit-runtime-acceptance", "context", DIGEST_B
            ),
        ),
        ...
        system_manifest_ref=HarnessBoundReference(
            "system-manifest:repository-repair-edit-runtime-acceptance",
            "system-manifest",
            DIGEST_C,
        ),
...
def _turn(
    ...
    return AgentTurnResult(
        model_call_id=f"model-call:repository-repair-edit-acceptance:{sequence}",
...
def _premature_completion_turn() -> AgentTurnResult:
    return AgentTurnResult(
        model_call_id="model-call:repository-repair-edit-acceptance:premature",
        model_id=ScriptedTurnAdapter.model_id,
...
def _completion_turn(workspace_id: str) -> AgentTurnResult:
    return AgentTurnResult(
        model_call_id="model-call:repository-repair-edit-acceptance:8",
        model_id=ScriptedTurnAdapter.model_id,
        ...
        usage={"inputTokens": 10, "outputTokens": 5},
        finish_reason="stop",
        raw_response_digest=canonical_digest({"editCandidateCompleted": True}),
    )
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(
        prefix="ordivon-repository-repair-edit-runtime-acceptance-"
    ) as directory:
        root = Path(directory)
        source = root / "source"
        state = root / "harness"
        ...
        try:
            ...
            with SQLiteHarnessStore.initialize(state) as store:
                ...
                calls = (
                    AgentToolCall(
                        "tool-call:repository-repair-edit-acceptance:read",
                        "read_workspace",
                        {"relativePath": "allocation.py", "mode": "FULL"},
                    ),
                    AgentToolCall(
                        "tool-call:repository-repair-edit-acceptance:replace",
                    ...
                    AgentToolCall(
                        "tool-call:repository-repair-edit-acceptance:create",
                    ...
                    AgentToolCall(
                        "tool-call:repository-repair-edit-acceptance:check",
                        "run_check",
                        {
                            "checkId": "visible-tests",
                            "waitMs": 30_000,
                    ...
                    AgentToolCall(
                        "tool-call:repository-repair-edit-acceptance:diff",
                        "diff_workspace",
                        {"maxBytes": 65_536},
                    ),
                    AgentToolCall(
                        "tool-call:repository-repair-edit-acceptance:reread",
                        "read_workspace",
                        {"relativePath": "allocation.py", "mode": "FULL"},
                    ),
                )
                ...
                visible_check = observations.get("run_check")
                source_replacement = observations.get("replace_workspace_text")
                completion_file = observations.get("create_workspace_file")
                diff_observation = observations.get("diff_workspace")
                checks = {
                    "candidateCompleted": result.candidate_completed,
                    "sixToolCalls": result.tool_calls == 6,
                    "eightModelCalls": result.model_calls == 8,
                    ...
                    "visibleCheckSucceeded": (
                        visible_check is not None
                        and visible_check.status == "observed"
                        and visible_check.runtime_job_ref is not None
                        and visible_check.structured_content.get("status") == "succeeded"
                    ),
                    "sourceReplacementApplied": (
                        source_replacement is not None
                        and source_replacement.status == "observed"
                    ),
                    "completionFileCreated": (
                        completion_file is not None
                        and completion_file.status == "observed"
                    ),
                    "diffObserved": (
                        diff_observation is not None
                        and diff_observation.status == "observed"
                    ),
                ...
                if not all(checks.values()):
                    diagnostic = {
                        ...
                        "stopDetail": (
                            result.trace.events[-1].payload.get("detail")
                        ...
                        "observationNames": sorted(observations),
                        "observationStatuses": {
                            name: item.status for name, item in sorted(observations.items())
                        },
                        "runtimeCalls": recorder.calls,
                        "traceKinds": [event.kind for event in result.trace.events],
                    }
                    raise RuntimeError(
                        "repository-repair edit acceptance checks failed: "
        ...
        return _with_integrity(
            {
                "schemaVersion": 1,
                "kind": "ordivon.harness-repository-repair-edit-runtime-acceptance",
...
def main() -> int:
    ...
    try:
        ...
        print(f"repository-repair edit Runtime acceptance: {type(error).__name__}: {error}")
        return 1
