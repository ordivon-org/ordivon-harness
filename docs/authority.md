...
[`../README.md`](../README.md) is the canonical repository entry. [`../ARCHITECTURE.md`](../ARCHITECTURE.md) owns the current Harness architecture, responsibility split, execution lifecycle, dispatch fencing, recovery ordering, and supported application surface.
...
Only the repository entry, architecture, and this decision enter strict content management in this adoption step. Stage-oriented texts remain available without bulk conversion or deletion. A later human-centered rewrite may turn the strongest evidence into clearer concepts, operations, examples, and troubleshooting paths, but it must preserve provenance and declare supersession rather than create a second current Harness model.
