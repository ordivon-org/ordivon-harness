...
- experimental task-conditioned capability discovery can return a bounded candidate set and later expand one exact descriptor instead of projecting the complete installed catalog. Candidate ranking is navigation only: discovery does not prove currentness, grant authority or mutate an active Run.
- experimental capability-to-First-Interface composition intersects discovered candidates with supplied owner/caller standing and the already-admitted action set. Missing standing remains `UNKNOWN`; only `AVAILABLE` actions already admitted by the existing turn surface can become current invokable affordances.
- preregistered fresh-Agent ablations now separate retrieval from currentness pressure. A 64-Tool static-semantic V1 was ceilinged (A/B/C all 12/12), so no generic retrieval behavior gain is claimed. A distinct V2 with byte-identical target/stale carrier descriptions produced A 5/11, B 3/12 and compact-current-affordance C 11/12 current-target selection with zero stale-twin selections; this supports a bounded behavioral consequence for current standing + existing admission, not semantic retrieval superiority.
- capability-aware First Interface now uses a compact model projection (`capabilityId`, owner, action, standing, admitted, `canInvokeNow`) while retaining the full summary/evidence/reason/digest carrier for exact inspection. It no longer repeats a parallel legacy affordance list, and admission drift after capability compilation fails closed.
...
- `build_observation_tool_surface()` is the advanced observation-only source-recovery composition. Its dynamic Grant binds exact search scope and readable source path/digest, can bind caller-owned owner/authority/version/transport evidence into the Run, and can project statements for one exact `subjectRef` from an immutable research-owner publication only after the publication's complete-file digest equals the bound `AuthorityVersionRef`. Search results are reduced to admitted object routing rather than used as line-level semantic evidence. Harness verifies and transports this fence but explicitly does not mint owner/currentness truth or grant mutation.
...
- Capability discovery is currently a bounded deterministic lexical/navigation mechanism over already-published descriptors, not a semantic ranker, embedding index, global registry or owner-currentness service. Large heterogeneous capability ecologies still need caller/owner descriptor publication and stronger retrieval evaluation before this surface can be promoted.
- Fresh-Agent V1/V2 does not earn a semantic retriever challenger: retrieval-only did not improve the controlled semantic-twin pressure. Reopen retrieval-backend research only when a real large capability ecology produces a discriminating retrieval failure; current engineering pressure is compact consumption of owner-current standing.
- Initial source discovery/ranking beyond that bounded descriptor navigation, large-WorkingSet inspection strategy, reusable-source repository ownership, procedure evaluation and canonical promotion remain Agent/application/Host/domain questions. Harness provides only advanced exact reference-to-seed admission for already-selected cross-Run sources.
