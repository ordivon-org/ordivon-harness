...
The mature Loop may also receive an ephemeral Working View projector. Provider disclosure still follows that projected view, but durable retention remains governed independently by the Contract. A committed WorkingView may be augmented for one turn only by exact model projections of Tool Observations already bound in that Provider Call's Run state; the selected WorkingView remains the exact prefix. This makes transient discovery evidence visible without silently promoting it into durable Agent-selected Context and prevents a projector from using the overlay seam as an arbitrary user/system-prompt injection channel.

A domain integration may interpret a raw Tool Observation and materialize discovered content through `store_working_view_source` before returning/enriching the exact Tool Observation that the Agent will see. The Harness verifies the materialization/privacy boundary and the observation/request identity chain; it does not decide which raw records are candidates or whether the domain's interpretation is semantically correct.

An ephemeral observation overlay used by a `metadata-only` Run is not made durable merely because it became Provider input; only the content authorized by the Contract is retained. An Agent-owned WorkingSet transition is different: it is durable cognition history and requires model-content authority. If the transition was chosen from a Tool-observation overlay, exact cross-process proof additionally requires Tool-content authority so the bound Provider request/state can retain and revalidate that overlay. `metadata-only` therefore cannot silently convert transient discovery bytes into a durable successor WorkingSet.
