...
Provider execution-control metadata is a separate disclosure channel from task Context. The DeepSeek adapter currently projects exact per-turn remaining budget, the names of currently admitted Runtime Tools, and—when enabled—the exact caller-ingress indexes/Provider positions currently eligible for promotion from `AgentTurnRequest` as a Harness-authored system control message. This metadata is not inserted into the WorkingSet or WorkingView and is not evidence about the task world; it tells the Agent which execution laws and cognition capabilities are currently in force. When no caller ingress is promotable, the promotion control itself is withdrawn rather than merely leaving an unusable function on the Provider surface.
