...
If caller-ingress promotion is explicitly enabled, the exact promoted source bytes still come only from already-authorized model content. The Agent proposal contains indexes and a successor slot, not caller text, summaries or caller-supplied digests. Continuity verifies those indexes against the exact Provider request's caller-ingress provenance, derives a deterministic source identity from the Run, pause boundary, indexes and exact messages, stores the source under ordinary WorkingView-source privacy rules, and extends the current WorkingSet atomically. `allow_model_content=True` is sufficient when no Tool channel is involved; a surrounding Tool-bearing transcript still requires its normal Tool-content authority. Metadata-only Runs cannot reconstruct or promote caller bytes they deliberately did not retain.

After promotion, Continuity may suppress the caller-overlay copy of an exact promoted index only while the corresponding promoted source remains selected in the current WorkingSet. This avoids duplicate Provider disclosure without merging provenance or deleting caller authority. Dropping the promoted pin re-exposes the same caller message during the still-active interaction; the next `needs_input` Snapshot expires the caller layer. Harness never performs semantic text deduplication and does not infer that two equal strings have the same provenance.

A domain integration may interpret a raw Tool Observation and materialize discovered content through `store_working_view_source` before returning/enriching the exact Tool Observation that the Agent will see. Materialization is a storage fact, not a cognition grant: an exact source may remain physically present in CAS while never appearing in any future model view. Only an Agent-authored successor WorkingSet that explicitly selects the source pin makes it durable current cognition; later omission of that pin removes its current cognition authority even though the source object still exists. The same rule governs correction: promoting a new caller fact does not semantically invalidate an older selected source, and Harness does not auto-delete the older source because text looks newer or contradictory. The Agent must explicitly select the corrected successor WorkingSet; omitted stale pins remain historical evidence rather than current cognition. Harness verifies structural materialization/privacy and exact identity boundaries; it does not decide which raw records deserve promotion or whether the domain's interpretation is semantically correct.
...
Provider execution-control metadata is a separate disclosure channel from task Context. The DeepSeek adapter currently projects exact per-turn remaining budget, the names of currently admitted Runtime Tools, and—when enabled—the exact caller-ingress indexes/Provider positions currently eligible for promotion from `AgentTurnRequest` as a Harness-authored system control message. When WorkingSet transitions are enabled it also discloses the exact current selected pins and the half-open Provider-message range produced by each pin, because an Agent cannot lawfully retain or drop a durable source it cannot address. These identities are model-visible only on the transition-capable Provider surface; they are not inserted into WorkingSet source messages or treated as task evidence. Continuity derives them from the current committed WorkingSet and exact source objects and rejects forged pin/range metadata before Provider dispatch. This identity disclosure requires the Run's existing model-content authority; it does not require Tool-content authority merely because the metadata is carried in the Harness-authored system control channel. When no caller ingress is promotable, the promotion control itself is withdrawn rather than merely leaving an unusable function on the Provider surface.
