...
The mature Loop may also receive an ephemeral Working View projector. Provider disclosure still follows that projected base view, but transient Tool continuity is owned by the Loop: for the current WorkingSet attempt it may append a complete Provider-authored Tool exchange consisting of the exact assistant `toolCalls` message followed by the exact bound Tool Observation messages. The selected WorkingView remains the exact prefix. An isolated Tool Observation, arbitrary user/system text, or replacement projection is not valid transient cognition evidence. On any fresh-process resume with authorized exact model and Tool content, Continuity uses the current committed WorkingSet Journal event as the attempt boundary and reconstructs only later Provider-authored Tool exchanges from exact Provider results plus terminal Tool intent/receipt/Observation evidence. This includes both active-effect crash recovery and ordinary clean pause/resume. Digest-only effect evidence still fences redispatch but is deliberately insufficient to recreate omitted cognition bytes.
