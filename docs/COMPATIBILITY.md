...
`ordivon_harness.api` is the recommended application facade. The package root mirrors that API plus `package_version`. `ordivon_harness.core` is the wider Host-free integration surface. P0 adds `HarnessAgentRun.explain()` on the existing recommended Run handle. Generated capability projectors live in advanced `ordivon_harness.capability_catalog`, exact cross-Run reusable knowledge/procedure reference-to-seed admission lives in advanced `ordivon_harness.knowledge_topology`, and the explicit application-local `HarnessAgentRunToolSurface` lives in `ordivon_harness.run_tool_surface`; none is added to the stable package-root facade yet. These advanced surfaces are admission/projection mechanisms rather than durable repository, ranking, semantic-evaluation or promotion authority.
