...
SPEC = {
    "schemaVersion": 0,
    "experimentId": "experiment:harness-research",
    "question": "What follows from this PIT cut?",
    ...
    "inputs": [
        {
            ...
            "datasetVersion": 0,
            "asOf": None,
            "knowledgeTimeField": "observed_at",
            "latestBy": [],
            "latestOrderField": "observed_at",
        }
    ],
    "parameters": {},
    "extensions": {},
...
def finance_success(*, research_status="observed", padding=0) -> dict[str, JsonValue]:
    ...
    if padding:
        result_value["padding"] = "x" * padding
    return {
        "schemaVersion": 1,
        "kind": "ordivon.finance.runtime-domain-result",
        "domain": "finance",
        "operation": "finance.research",
        "interfaceVersion": 3,
        "ok": True,
        "effectContract": {
            "schemaVersion": 1,
            ...
            "effectClass": "CANONICAL_RESEARCH",
            "credentialAccess": "none",
            "environmentMutation": False,
            "externalWorldRead": False,
        ...
        "result": {
            "schemaVersion": 0,
            "kind": "ordivon.finance.semantic-research-result",
            "status": research_status,
            "semanticCallId": "harness-finance-research:fixture",
            "sourceFence": {},
            "researchRunnerWorkspaceId": RUNNER_WS,
            "runtimeJobRef": "runtime://job/inner-job",
            "reconciled": True,
            "stateVersionBefore": "10:aaa",
            "stateVersionAfter": "13:bbb",
            "capitalStateCountsBefore": {
                "decisions": 1,
                "proposals": 2,
                "externalEffects": 3,
                "capitalLedgerEntries": 4,
            },
            "capitalStateCountsAfter": {
                "decisions": 1,
                "proposals": 2,
                "externalEffects": 3,
                "capitalLedgerEntries": 4,
            ...
            "consumerStanding": {
                "semanticOperation": "finance.research",
                "canonicalResearchStateMutation": True,
                "decisionMutation": False,
                "proposalMutation": False,
                "externalEffectMutation": False,
                "capitalLedgerMutation": False,
                "externalWorldRead": False,
                "venueCredentialAccess": False,
                "externalFinancialWriteAttempted": False,
                "financialSubmissionAttempted": False,
                "authorityMutation": False,
                "runtimePhysicalExecution": True,
            },
            "evidenceRef": "evidence://sha256/" + "1" * 64,
            "experimentId": "experiment:harness-research",
            "semanticResultDigest": "sha256:" + "2" * 64,
            "sourceStateVersion": "10:aaa",
            "materializationId": "research-input-materialization://sha256/" + "3" * 64,
            "runtimeJobId": "inner-job",
            "runtimeAttemptId": "inner-attempt",
            "admissionCapability": "research.result.admit@5",
            "replayed": False,
            "result": result_value,
        },
    }
...
class FinanceResearchFakeRuntime:
    ...
    def terminal(self) -> dict[str, JsonValue]:
        ...
        return {
            ...
            "artifacts": [
                {
                    "artifactId": "finance-research.stdout",
                    "kind": "stdout",
            ...
            "stdoutTail": stdout[-65536:] if len(raw) > 65536 else stdout,
            "stderrTail": "",
        }
    ...
    def call_tool(self, name: str, arguments: dict[str, JsonValue]) -> dict[str, JsonValue]:
        ...
        if name == "workspace.exec":
            ...
            self.client_request_id = arguments["clientRequestId"]
            if self.mode == "loss":
...
def contract(suffix: str) -> HarnessRunContract:
    return HarnessRunContract(
        harness_run_id=f"harness-run:finance-research-{suffix}",
        harness_implementation_id="ordivon-harness@test",
        caller_id="caller:test",
        caller_run_ref=f"trial:{suffix}",
        ...
        tool_catalog_digest=FINANCE_RESEARCH_TOOL_SURFACE_DIGEST,
        tool_grant_digest=grant().digest,
        budget={"maxModelCalls": 3, "maxToolCalls": 2, "maxWallTimeMs": 10_000},
        completion_contract={"mode": "record"},
        system_manifest_ref=HarnessBoundReference(f"system:{suffix}", "system-manifest", DIGEST_C),
        created_at_ms=1000,
    )
...
def execution_binding(
    ...
    refs = (
        ...
        HarnessRuntimeReference(
            ...
            reference_id=f"run:{run_contract.digest[7:31]}",
            generation="1",
        ...
        HarnessRuntimeReference(
            ...
            reference_id=f"grant:{run_contract.tool_grant_digest[7:31]}",
            generation="1",
...
def call(suffix: str, arguments=None) -> AgentToolCall:
    return AgentToolCall(
        tool_call_id=f"tool-call:finance-research-{suffix}",
        name="finance_research",
        arguments={"researchRunSpec": SPEC} if arguments is None else arguments,
    )
...
class FinanceResearchRuntimeBridgeTests(unittest.TestCase):
    def initialize(self, directory: str, suffix: str, runtime: FinanceResearchFakeRuntime):
        ...
        bridge = SQLiteHarnessFinanceResearchRuntimeBridge(
            rc, continuity, execution_binding(rc, continuity), runtime, grant()
        )
        bridge.bind_run_state(
            messages=({"role": "user", "content": "research the current Finance question"},),
            observations=(),
            remaining_budget={
                "modelCalls": 3,
                "modelRetries": 1,
    ...
    def test_surface_is_semantic_only_and_recovery_consequence_is_strong(self):
        self.assertEqual(
            set(FINANCE_RESEARCH_DEFINITION.input_schema["properties"]), {"researchRunSpec"}
        )
        self.assertFalse(FINANCE_RESEARCH_DEFINITION.input_schema["additionalProperties"])
        self.assertEqual(
            SQLiteHarnessFinanceResearchRuntimeBridge.recovery_consequence,
            HarnessRecoveryConsequence.PROCESS_OR_EXTERNAL_EFFECT_POSSIBLE,
        )
        self.assertEqual(
            SQLiteHarnessFinanceResearchRuntimeBridge.observation_only_tool_names, frozenset()
        )
        g = grant().to_dict()
        self.assertFalse(g["financialWriteAllowed"])
        self.assertFalse(g["providerCredentialAllowed"])

    def test_lowering_keeps_research_equipment_out_of_agent_arguments(self):
        with tempfile.TemporaryDirectory() as directory:
            runtime = FinanceResearchFakeRuntime()
            _, _, bridge = self.initialize(directory, "lower", runtime)
            ...
            self.assertEqual(execution["workspaceId"], OWNER_WS)
            self.assertEqual(execution["executable"], "/usr/bin/node")
            self.assertEqual(
                execution["args"][:4],
                ["scripts/finance-domain.mjs", "call", "--operation", "finance.research"],
            )
            owner_args = json.loads(execution["args"][5])
            self.assertEqual(owner_args, {"researchRunSpec": SPEC})
            env = execution["env"]
            ...
            self.assertEqual(
                env["ORDIVON_FINANCE_RESEARCH_TRUSTED_IMPLEMENTATION_DIGEST"], IMPLEMENTATION
            )
            self.assertTrue(
                env["ORDIVON_FINANCE_RESEARCH_CALL_ID"].startswith("harness-finance-research:")
            )
            ...
            self.assertFalse(
                observation.structured_content["effectBoundary"]["externalFinancialWrite"]
            )

    def test_float_finance_result_is_preserved_as_json_text_without_violating_harness_canonical_values(
        self,
    ):
        value = {"ratio": 0.0547945205479452, "nested": [{"costBps": 10.183794520547945}]}
        text, omitted, digest, size = SQLiteHarnessFinanceResearchRuntimeBridge._project_result(
            value
        )
    ...
    def test_runtime_response_loss_reattaches_outer_job_without_redispatch(self):
        with tempfile.TemporaryDirectory() as directory:
            runtime = FinanceResearchFakeRuntime("loss")
            _, _, bridge = self.initialize(directory, "loss", runtime)
    ...
    def test_large_research_result_uses_verified_artifact_and_bounded_projection(self):
        with tempfile.TemporaryDirectory() as directory:
            runtime = FinanceResearchFakeRuntime("large")
            _, _, bridge = self.initialize(directory, "large", runtime)
            ...
            projection = observation.structured_content["financeProjection"]
            self.assertTrue(projection["resultOmittedByHarnessBound"])
            self.assertIsNone(projection["resultJson"])
            self.assertGreater(projection["resultBytes"], 128_000)
            self.assertTrue(projection["resultDigest"].startswith("sha256:"))

    def test_unknown_agent_deployment_argument_is_rejected_before_runtime(self):
        with tempfile.TemporaryDirectory() as directory:
            runtime = FinanceResearchFakeRuntime()
            _, _, bridge = self.initialize(directory, "bad", runtime)
            with self.assertRaisesRegex(Exception, "only one researchRunSpec"):
                bridge.execute(
                    call("bad", {"researchRunSpec": SPEC, "runnerExecutable": "/tmp/agent"}),
                    step_id="step:bad",
                )
