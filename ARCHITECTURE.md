...
A transient model turn may append discovery evidence to the committed Working View without silently selecting it. On the durable Continuity path, the committed Working View must remain the exact request prefix and every appended overlay message must be an exact model projection of a Tool Observation already bound in that Provider Call's Run state, in observation order. Arbitrary user/system text and replacement projections are rejected before physical Provider dispatch. Continuity Doctor replays the same rule from durable Provider request/state evidence, so a mechanically valid low-level event cannot invent an overlay provenance after the fact.
...
Continuity applies one accepted proposal as an atomic `replan → select exact pins → commit` Journal transaction under one Run lease. Provider-authored transitions bind two different facts: `sourceWorkingSetDigest` identifies the durable selected Context that remained current, while `sourceModelViewDigest` identifies the exact effective view the Agent actually saw, including any admitted transient observation overlay. The exact retained Provider request/result prove that the proposal came from that model view. Competing proposals have one winner, exact replay is idempotent, and Continuity Doctor verifies the proposal/source/Provider evidence rather than trusting a mechanically valid WorkingSet chain alone. Base-only direct transition receipts remain readable under the earlier single-view evidence form.

After a successor WorkingSet commits, overlays associated with the predecessor are not part of that new WorkingSet; a domain projector must therefore stop projecting them. The validated experiments key the transient overlay to its source WorkingSet digest, so the next Provider turn after selecting B sees B rather than A plus stale discovery evidence.
