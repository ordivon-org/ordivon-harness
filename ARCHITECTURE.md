...
After a successor WorkingSet commits, the prior attempt's transient Tool exchanges are cleared rather than promoted into the successor WorkingSet. The next Provider turn after selecting B therefore sees B rather than A plus stale discovery history. Cross-process recovery now preserves the same law at a durable Tool-effect boundary: a fresh process identifies the current committed WorkingSet event as the attempt boundary, reconstructs only later completed Provider Tool turns, verifies their exact Tool calls against terminal Tool intent/receipt/Observation evidence, reconciles an already-completed active Tool without redispatch, executes only still-pending calls, and rebuilds the complete Provider-faithful exchange before the next model turn. No new conversation store or SQLite schema is required; the recovery projection is derived from existing Journal/CAS authority. This slice does not claim arbitrary recovery from every possible local-only correction or from crash points for which no authorized exact Tool content was durably retained.
...
No-progress and observation-only limits are effect gates, not epistemic verdicts. Reaching one closes new external observation capability and gives the Agent a subsequent cognition/closure turn over the evidence it has already seen. Exhausting the hard Tool budget does the same. A Provider that still refers to a now-unavailable historical Runtime Tool receives a bounded local, model-correctable rejection with `physicalDispatch=false`; this correction consumes correction budget but cannot exceed the external Tool budget. Harness therefore bounds effects while the Agent still owns whether to conclude `candidate_completed`, report concrete `needs_input`, or perform an admitted cognition transition. Only repeated failure to respect the closed gate falls back to a mechanical stop.

The projector, transition handler, overlay helper and DeepSeek transition switch remain experimental execution seams rather than part of the recommended `ordivon_harness.api` facade. The proposal value is available from `ordivon_harness.core` for caller-neutral integrations. Harness still has no generic CandidateSet, Context Service or search/relevance policy.
